...
public class Interval implements Comparable<Interval> {
	 ...
	 */
	public static final Interval INFINITY = new Interval();
	 ...
	 * The interval {@code [-1, -1]}.
	 */
	public static final Interval MINUS_ONE = new Interval(-1, -1);
	...
	private static Interval cacheAndRound(
			Interval i) {
		if (i.is(0))
			return ZERO;
		if (i.is(1))
		...
		if (i.is(-1))
			return MINUS_ONE;
		return new Interval(i.low, i.high);
	}
	 ...
	 */
	public Interval plus(
			Interval other) {
		if (isInfinity() || other.isInfinity())
			return INFINITY;

		return cacheAndRound(new Interval(low.add(other.low), high.add(other.high)));
	}
	 ...
	 * @return {@code this - other}
	 */
	public Interval diff(
			Interval other) {
		if (isInfinity() || other.isInfinity())
			return INFINITY;

		return cacheAndRound(new Interval(low.subtract(other.high), high.subtract(other.low)));
	}
	...
	public Interval mul(
		...
		if (low.compareTo(MathNumber.ZERO) >= 0 && other.low.compareTo(MathNumber.ZERO) >= 0)
			return cacheAndRound(new Interval(low.multiply(other.low), high.multiply(other.high)));
		...
		return cacheAndRound(new Interval(min(ll, lh, hl, hh), max(ll, lh, hl, hh)));
	}
	...
	public Interval div(
		...
		if (!other.includes(ZERO))
			return mul(new Interval(MathNumber.ONE.divide(other.high), MathNumber.ONE.divide(other.low)));
		else if (other.high.isZero())
			return mul(new Interval(MathNumber.MINUS_INFINITY, MathNumber.ONE.divide(other.low)));
		else if (other.low.isZero())
			return mul(new Interval(MathNumber.ONE.divide(other.high), MathNumber.PLUS_INFINITY));
		else if (ignoreZero)
			return mul(new Interval(MathNumber.ONE.divide(other.low), MathNumber.ONE.divide(other.high)));
		else {
			Interval lower = mul(new Interval(MathNumber.MINUS_INFINITY, MathNumber.ONE.divide(other.low)));
			Interval higher = mul(new Interval(MathNumber.ONE.divide(other.high), MathNumber.PLUS_INFINITY));
			...
			if (lower.includes(higher))
				...
				return cacheAndRound(new Interval(lower.low.compareTo(higher.low) > 0 ? higher.low : lower.low,
