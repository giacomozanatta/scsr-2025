...
public class Intervals
	...
	public Intervals evalUnaryExpression(UnaryOperator operator, Intervals arg, ProgramPoint pp, SemanticOracle oracle)
		...
		if (operator instanceof NumericNegation) {
			...
			BigDecimal ilo = i.getLow().getNumber().negate();
			BigDecimal ihi = i.getHigh().getNumber().negate();

			return new Intervals(new MathNumber(ihi), new MathNumber(ilo));
		}
	...
	public Intervals evalBinaryExpression(BinaryOperator operator, Intervals left, Intervals right, ProgramPoint pp,
		...
		MathNumber llo = left.interval.getLow();
		MathNumber lhi = left.interval.getHigh();

		MathNumber rlo = right.interval.getLow();
		MathNumber rhi = right.interval.getHigh();
		
		if (operator instanceof AdditionOperator)  {
			return new Intervals(llo.add(rlo), lhi.add(rhi));
			
		} else if (operator instanceof SubtractionOperator) {
			return new Intervals(llo.subtract(rhi), lhi.subtract(rlo));

		} else if (operator instanceof MultiplicationOperator) {
			return manageMultiplications(llo, lhi, rlo, rhi);

		} else if (operator instanceof DivisionOperator) {

			if (rlo.equals(MathNumber.ZERO) && rhi.equals(MathNumber.ZERO)) {
				return TOP;
			}

			MathNumber rloInv = rlo.equals(MathNumber.ZERO) ? MathNumber.PLUS_INFINITY : MathNumber.ONE.divide(rlo);
			MathNumber rhiInv = rhi.equals(MathNumber.ZERO) ? MathNumber.MINUS_INFINITY : MathNumber.ONE.divide(rhi);

			return manageMultiplications(llo, lhi, rloInv, rhiInv);
		}

		return top();
	}

	private Intervals manageMultiplications(MathNumber llo, MathNumber lhi, MathNumber rlo, MathNumber rhi) {
		MathNumber[] combinations = new MathNumber[] {
				llo.multiply(rlo),
				llo.multiply(rhi),
				lhi.multiply(rlo),
				lhi.multiply(rhi)
		...
		List<MathNumber> all = new ArrayList<>(Arrays.asList(combinations));
		Collections.sort(all);
		// take the smallest and the biggest
		return new Intervals(all.get(0).roundDown(), all.get(3).roundUp());
