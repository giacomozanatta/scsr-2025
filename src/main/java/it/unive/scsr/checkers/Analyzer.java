...
public class Analyzer<V extends ValueDomain<V>> {
	...
	public List<SymbolicExpression> getComputedExpression(Statement statement) {
		var expressionsSpliterator = analyzer.getAnalysisStateAfter(statement).getComputedExpressions().spliterator();
	...
	public static Set<Type> inferTypes(SymbolicExpression expression, ProgramPoint programPoint, SemanticOracle oracle) {
		try {
			...
			return oracle.getRuntimeTypesOf(expression, programPoint, oracle).stream().filter(t -> t != null && t != Untyped.INSTANCE).collect(Collectors.toSet());
		} catch (SemanticException e) {
	...
	public Set<SymbolicExpression> reachableElements(SymbolicExpression expression, ProgramPoint programPoint, SemanticOracle oracle) {
		try {
	...
	public static boolean anyNumericalType(SymbolicExpression expression, ProgramPoint programPoint, SemanticOracle oracle) {
		...
		return inferTypes(expression, programPoint, oracle).stream().map(Type::getClass).anyMatch(NumericType.class::isAssignableFrom);
	}

	public static <T extends NonRelationalValueDomain<T>> T evalOrThrow(ValueEnvironment<T> environment, ValueExpression expression, ProgramPoint programPoint, SemanticOracle oracle) {
		try {
