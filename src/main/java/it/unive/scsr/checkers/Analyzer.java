...
public class Analyzer<V extends ValueDomain<V>> {
  ...
  private final AnalyzedCFG<SimpleAbstractState<PointBasedHeap, V, TypeEnvironment<InferredTypes>>>
      analyzer;

  public Analyzer(
      AnalyzedCFG<SimpleAbstractState<PointBasedHeap, V, TypeEnvironment<InferredTypes>>>
          analyzer) {
  ...
  public List<SymbolicExpression> getComputedExpression(Statement statement) {
    var expressionsSpliterator =
        analyzer.getAnalysisStateAfter(statement).getComputedExpressions().spliterator();
   ...
   */
  public static Set<Type> inferTypes(
      SymbolicExpression expression, ProgramPoint programPoint, SemanticOracle oracle) {
    try {
   ...
   * @param oracle The oracle for inter-domain communication.
   * @return The set of all reachable expressions or an empty set if something goes wrong during the
   *     computation.
   */
  public Set<SymbolicExpression> reachableElements(
      SymbolicExpression expression, ProgramPoint programPoint, SemanticOracle oracle) {
    try {
   ...
   * @param programPoint The program point where the types are required.
   * @param oracle The oracle for inter-domain communication.
   * @return <code>true</code> when at least one of the inferred types is numeric, <code>false
   *     </code> otherwise.
   */
  public static boolean anyNumericalType(
      SymbolicExpression expression, ProgramPoint programPoint, SemanticOracle oracle) {
    // The set of inferred types must contain at least one NumericType.
    return inferTypes(expression, programPoint, oracle).stream()
        .map(Type::getClass)
