					...
					for (SymbolicExpression s : reachableIds) {
						...
						if (intervalAbstractValue != null && !intervalAbstractValue.isBottom()) {
							...
							if (intervalAbstractValue.interval != null) {
								try {
									boolean minZero = intervalAbstractValue.interval.getLow().isFinite() && intervalAbstractValue.interval.getLow().toLong() == 0;
									boolean maxZero = intervalAbstractValue.interval.getHigh().isFinite() && intervalAbstractValue.interval.getHigh().toLong() == 0;
									long min = intervalAbstractValue.interval.getLow().isFinite() ? intervalAbstractValue.interval.getLow().toLong() : Long.MIN_VALUE;
									long max = intervalAbstractValue.interval.getHigh().isFinite() ? intervalAbstractValue.interval.getHigh().toLong() : Long.MAX_VALUE;
									if (min <= 0 && max >= 0)
										canBeZero = true;
								} catch (Exception e) {
							...
							if (canBeZero) {
								System.out.println("Possible division by zero detected at: " + div.getLocation());
							}
						}
					}
