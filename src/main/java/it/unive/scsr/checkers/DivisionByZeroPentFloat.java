...
public class DivisionByZeroPentFloat implements SemanticCheck<SimpleAbstractState<PointBasedHeap, FloatPentagons, TypeEnvironment<InferredTypes>>> {
    private OverflowChecker.NumericalSize size;

    public DivisionByZeroPentFloat(OverflowChecker.NumericalSize size){this.size = size;}

    @Override
    public boolean visit(CheckToolWithAnalysisResults<SimpleAbstractState<PointBasedHeap, FloatPentagons, TypeEnvironment<InferredTypes>>> tool,
                         CFG graph,
                         Statement node) {
    ...
    private void checkDivisionPent(CheckToolWithAnalysisResults<SimpleAbstractState<PointBasedHeap, FloatPentagons, TypeEnvironment<InferredTypes>>> tool,
            ...
            if(comExprIterator.hasNext()) {
                ...
                try {
                    ...
                    for (SymbolicExpression s : reachableIds) {
                        ...
                        FloatPentagons valueState = state.getState().getValueState();
                        ValueEnvironment<FloatIntervals> valueStateIntervals = valueState.getIntervals();
                        FloatIntervals intervalAbstractValue = valueStateIntervals.eval((ValueExpression) s, div, state.getState());
    ...
    private Set<Type> getPossibleDynamicTypes(SymbolicExpression s, Division div,
                                              SimpleAbstractState<PointBasedHeap, FloatPentagons, TypeEnvironment<InferredTypes>> state) throws SemanticException {
