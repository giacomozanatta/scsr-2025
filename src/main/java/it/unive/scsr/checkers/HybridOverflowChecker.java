...
public class HybridOverflowChecker implements
        SemanticCheck<
    ...
    public HybridOverflowChecker(NumericalSize size) {
        this.size = size;
    }
    ...
    private void checkVariableRef(CheckToolWithAnalysisResults<SimpleAbstractState<PointBasedHeap, ValueEnvironment<HybridIntervals>, TypeEnvironment<InferredTypes>>> tool, VariableRef varRef, CFG graph, Statement node ) {
        ...
        for (AnalyzedCFG<SimpleAbstractState<PointBasedHeap, ValueEnvironment<HybridIntervals>,
            ...
            SimpleAbstractState<PointBasedHeap, ValueEnvironment<HybridIntervals>, TypeEnvironment<InferredTypes>> state = result.getAnalysisStateAfter(node).getState();
            HybridIntervals intervalAbstractValue = state.getValueState().getState(id);
            ...
            try {
                if(intervalAbstractValue.interval != null) {
                    ...
                    if (lb < min) {
                        if (ub < min)
                            tool.warnOn(node, String.format("[%s] Underflow DETECTED: %.2f < %.2f for variable %s",
                                    size, ub, min, id.getName()));
                        else
                            tool.warnOn(node, String.format("[%s] POSSIBLE underflow DETECTED: %.2f < %.2f for variable %s",
                                    size, lb, min, id.getName()));
                    }
                    if (ub > max) {
                        if (lb > max)
                            tool.warnOn(node, String.format("[%s] Overflow DETECTED: %.2f > %.2f for variable %s",
                                    size, lb, max, id.getName()));
                        else
                            tool.warnOn(node, String.format("[%s] POSSIBLE overflow DETECTED: %.2f > %.2f for variable %s",
    ...
    private Set<Type> getPossibleDynamicTypes(
        ...
        for (AnalyzedCFG<
                SimpleAbstractState<PointBasedHeap, ValueEnvironment<HybridIntervals>,
                        TypeEnvironment<InferredTypes>>> result : tool.getResultOf(graph)) {
            SimpleAbstractState<PointBasedHeap, ValueEnvironment<HybridIntervals>, TypeEnvironment<InferredTypes>> state = result.getAnalysisStateAfter(varRef).getState();
            try {
