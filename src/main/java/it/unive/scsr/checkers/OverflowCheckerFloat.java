...
public class OverflowCheckerFloat implements
        SemanticCheck<
    ...
    public OverflowCheckerFloat(OverflowChecker.NumericalSize size) {
        this.size = size;
    }
    ...
    private void checkVariableRef(CheckToolWithAnalysisResults<SimpleAbstractState<PointBasedHeap, ValueEnvironment<FloatIntervals>, TypeEnvironment<InferredTypes>>> tool, VariableRef varRef, CFG graph, Statement node ) {
        ...
        for (AnalyzedCFG<SimpleAbstractState<PointBasedHeap, ValueEnvironment<FloatIntervals>,
            ...
            SimpleAbstractState<PointBasedHeap, ValueEnvironment<FloatIntervals>, TypeEnvironment<InferredTypes>> state = result.getAnalysisStateAfter(target).getState();
            FloatIntervals intervalAbstractValue = state.getValueState().getState(id);
    ...
    private Set<Type> getPossibleDynamicTypes(
        ...
        for (AnalyzedCFG<
                SimpleAbstractState<PointBasedHeap, ValueEnvironment<FloatIntervals>,
                        TypeEnvironment<InferredTypes>>> result : tool.getResultOf(graph)) {
            SimpleAbstractState<PointBasedHeap, ValueEnvironment<FloatIntervals>, TypeEnvironment<InferredTypes>> state = result.getAnalysisStateAfter(varRef).getState();
            try {
