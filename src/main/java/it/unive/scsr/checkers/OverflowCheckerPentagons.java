...
public class OverflowCheckerPentagons implements SemanticCheck<SimpleAbstractState<PointBasedHeap, Pentagons, TypeEnvironment<InferredTypes>>> {
    ...
    public OverflowCheckerPentagons(OverflowChecker.NumericalSize size) {
        this.size = size;
    }
    ...
    private void checkVariableRef(CheckToolWithAnalysisResults<SimpleAbstractState<PointBasedHeap, Pentagons, TypeEnvironment<InferredTypes>>> tool, VariableRef varRef, CFG graph, Statement node ) {
        ...
        for (AnalyzedCFG<SimpleAbstractState<PointBasedHeap, Pentagons,
            ...
            Pentagons valueState = result.getAnalysisStateAfter(node).getState().getValueState();
            ValueEnvironment<Intervals> valueStateIntervals = valueState.getIntervals();
            Intervals intervalAbstractValue = valueStateIntervals.getState(id);
