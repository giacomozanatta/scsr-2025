...
public class TaintChecker implements
		SemanticCheck<SimpleAbstractState<PointBasedHeap, ValueEnvironment<Taint>, TypeEnvironment<InferredTypes>>> {
	...
	// The idea is to detect calls with formal parameter annotated as sink and check
	...
	public boolean visit(
		...
		if (!(node instanceof UnresolvedCall))
			return true; // if it is a statement that it is not a call, then we don't care because cannot
										// have paramenters annaotated as sinks

		UnresolvedCall call = (UnresolvedCall) node;
		try {
			// we get the taint analysis results mapped on the CFG containg the call that we
			// want investigate
			for (AnalyzedCFG<SimpleAbstractState<PointBasedHeap, ValueEnvironment<Taint>, TypeEnvironment<InferredTypes>>> result : tool
					...
					for (CodeMember n : cfg.getTargets()) {
						...
						for (int i = 0; i < parameters.length; i++)
							if (parameters[i].getAnnotations().contains(SINK_MATCHER)) {
								...
								AnalysisState<SimpleAbstractState<PointBasedHeap, ValueEnvironment<Taint>, TypeEnvironment<InferredTypes>>> state = result
								...
								for (SymbolicExpression s : reachableIds) {
									...
									if (valueState.eval((ValueExpression) s, node, state.getState())
											.isPossiblyTainted())
										// in the sink flows a possible tainted data, then we report an warning in the
										// LiSA report result
