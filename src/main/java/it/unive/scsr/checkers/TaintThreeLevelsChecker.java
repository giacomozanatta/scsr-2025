...
public class TaintThreeLevelsChecker implements
        SemanticCheck<SimpleAbstractState<PointBasedHeap, ValueEnvironment<TaintThreeLevels>, TypeEnvironment<InferredTypes>>> {
    ...
    public boolean visit(
        ...
        try {
            for (AnalyzedCFG<SimpleAbstractState<PointBasedHeap, ValueEnvironment<TaintThreeLevels>, TypeEnvironment<InferredTypes>>> result : tool
                    ...
                    for (CodeMember n : cfg.getTargets()) {
                        ...
                        for (int i = 0; i < parameters.length; i++)
                            if (parameters[i].getAnnotations().contains(SINK_MATCHER)) {
                                AnalysisState<SimpleAbstractState<PointBasedHeap, ValueEnvironment<TaintThreeLevels>, TypeEnvironment<InferredTypes>>> state = result
                                ...
                                for (SymbolicExpression s : reachableIds) {
                                    ...
                                    if (valueState.eval((ValueExpression) s, node, state.getState()).isAlwaysTainted())
                                        tool.warnOn(call, "[DEFINITE] The value passed for the "
                                            ...
                                            .isPossiblyTainted())
                                        tool.warnOn(call, "[POSSIBLE] The value passed for the "
                                                + StringUtilities.ordinal(i + 1)
