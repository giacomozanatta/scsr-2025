...
public class BinaryFunctions extends BiFunctionDispatcher<Intervals> {
  ...
  protected BiFunction<Intervals, Intervals, Intervals> buildAdditionFunction() {
    return (left, right) ->
        left.interval.add(right.interval, null).map(left::changeInterval).orElse(left.top());
  }

  @Override
  protected BiFunction<Intervals, Intervals, Intervals> buildSubtractionFunction() {
    return (left, right) ->
        left.interval.subtract(right.interval, null).map(left::changeInterval).orElse(left.top());
  }
  ...
  protected BiFunction<Intervals, Intervals, Intervals> buildMultiplicationFunction() {
    ...
    // of returning the closest approximation, zero is returned.
    IntervalNumber.Computation computation =
        (first, second) -> {
          ...
          // between zero and any other IntervalNumber, including Infinity.
  ...
  protected BiFunction<Intervals, Intervals, Intervals> buildDivisionFunction() {
    return (left, right) -> {
      ...
      // division is by zero, that is the bottom element can be returned.
      if (right.interval.is(IntervalNumber.ofPrimitiveOrThrow(0))) return left.bottom();
