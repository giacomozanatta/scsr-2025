...
public abstract sealed class Numeric<T> implements SigNum permits IntegerNumber, DecimalNumber {
  ...
  protected static final int SCALE = 512;
  protected static final RoundingMode ROUNDING_MODE = RoundingMode.HALF_DOWN;
    ...
    return finalize(
        switch (other) {
          ...
          case Numeric<?> numeric ->
              numeric.isZero() ? NaN.INSTANCE : bestApproximation(numeric, Operation.DIVIDE);
        },
        other,
        orElse);
  }
  ...
  private Numeric<?> bestApproximation(Numeric<?> otherNumeric, Operation operation) {
    // Define a BiFunction to perform the given operation on two Numeric values, converting them to
    // DecimalNumber
    // first to ensure precision.
    BiFunction<Numeric<?>, Numeric<?>, DecimalNumber> decimalApproximation =
        (first, second) -> {
    ...
    // Use a switch expression based on the type of 'otherNumeric' to determine the best
    // approximation.
    return switch (otherNumeric) {
      case DecimalNumber ignored -> decimalApproximation.apply(this, otherNumeric);
      case IntegerNumber otherInteger ->
          switch (this) {
            case DecimalNumber ignored -> decimalApproximation.apply(this, otherNumeric);
            case IntegerNumber thisInteger ->
                new IntegerNumber(
                    numericMethod(BigInteger.class, operation)
