...
public abstract class BiFunctionDispatcher<T extends Lattice<T>> {
  ...
  // Protected functions that must be implemented by subclasses to create the correct function to
  // apply when an
  ...
  /**
   * Refine the output after the calculation has been performed. By default, this function returns
   * the output itself without any modification.
   */
  ...
  /**
   * Returns an optional element whenever the specified operands cannot be combined to produce a new
   * element. By default, the following logic is applied:
   *
   * <ol>
   *   <li>When left or right is bottom, bottom is returned
   *   <li>When left or right is top, top is returned
   * </ol>
   */
  protected Optional<T> inapplicable(T left, T right) {
    // As soon as one of the elements expresses a bottom element, the calculation is stopped and
    // then bottom is
    // returned.
    if (left.isBottom() || right.isBottom()) return Optional.of(left.bottom());

    // Whenever a top element is found, the calculation is stopped and then the bottom is returned.
    // This must be
    // done because the top element can also express something that is not an interval, so any
    // calculation can
  ...
  private BiFunction<T, T, T> buildWrapper(final BiFunction<T, T, T> biFunction) {
    return (left, right) -> {
      // If the computation cannot continue (e.g. the specified operand cannot be combined), the
      // "inapplicable"
      // element is returned, otherwise the computation proceeds by calling the specified binary
      // function and
      // refining the result.
      return inapplicable(left, right).orElse(polishResult(biFunction.apply(left, right)));
    };
  }
}
