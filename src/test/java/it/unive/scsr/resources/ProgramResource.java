...
public abstract class ProgramResource<S extends SemanticCheck<?>, V extends ValueDomain<V>> {
   ...
   */
  public Runnable toLiSARunner(Collection<S> checkers, V domain) {
    return () -> {
      ...
      configuration.interproceduralAnalysis =
          new ContextBasedAnalysis<>(FullStackToken.getSingleton());
      configuration.semanticChecks.addAll(checkers);

      // Compile LiSA and run this program.
      new LiSA(update(configuration)).run(update(ProgramsUtils.buildOrThrow(inputFile)));
    };
  }
   ...
   * @return
   */
  public LiSAConfiguration update(LiSAConfiguration configuration) {
