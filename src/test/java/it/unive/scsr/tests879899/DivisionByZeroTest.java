...
public class DivisionByZeroTest {
    ...
    // initialized with input and output directory paths.
    private static class DivisionByZeroProgram extends ProgramResource<DivisionByZeroChecker, ValueEnvironment<Intervals>> {
    ...
    @Test
    public void test() {
        // Defines a function that takes a Path and creates a new DivisionByZeroProgram instance, using the path's
        // string representation as the input file and a default output directory derived from the filename.
        Function<Path, DivisionByZeroProgram> overflowProgram =
                path -> new DivisionByZeroProgram(path.toString(), defaultOutputDirectory(removeExt(path.getFileName())));
        ...
        filenames(defaultInputDirectory())
                .stream()
                .map(overflowProgram)
                .map(program -> program.toLiSARunner(Set.of(new DivisionByZeroChecker()), new ValueEnvironment<>(Intervals.TOP), null))
