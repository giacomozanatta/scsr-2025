...
public class PentagonsOverflowTest {
  // A private static nested class representing a program resource specialized for overflow
  // checking, extending ProgramResource and parameterized with OverflowChecker and Pentagons. It is
  // initialized with input and output directory paths.
  private static class OverflowProgram
      extends ProgramResource<OverflowChecker<Pentagons>, Pentagons> {
  ...
  public void test() {
    ...
    var checkers =
        Arrays.stream(OverflowChecker.NumericalSize.values())
            .map(size -> new OverflowChecker<Pentagons>(size))
            .toList();
    ...
    filenames(defaultInputDirectory()).stream()
        .map(overflowProgram)
        .map(program -> program.toLiSARunner(checkers, new Pentagons()))
